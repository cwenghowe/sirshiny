...
shinyServer(function(input, output) {
    ...
    output$gammaSlider <- renderUI({
        sliderInput("gammaInit",
                    ...
                    min = round(1/42,4),
                    max = round(1/input$gammaUpper,4),
                    value = round(1/14,4))
    })
    ...
    output$sirplot <- renderPlot({
        ...
        generateSIR(input$N, input$betaInit, input$gammaInit, input$gammaUpper)

    },height=500)
    

    
    generateSIR <- function(susceptible, initBeta, initGamma,gammaUpper) {
